...
export function StaffAllHistory() {
  ...
  useEffect(() => {
    ...
    Promise.all([staffFetchAllProfiles(), staffFetchAllCheckIns()])
      ...
      .catch((err) => {
        if (!active) return;
        setError(
          err instanceof Error ? err.message : "Could not load tests."
        );
      })
      .finally(() => {
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-sage-600">
        {loading
          ? "Loading from cloud…"
          : `${checkIns.length} test${checkIns.length === 1 ? "" : "s"} from all users.`}
      </p>
      {error && (
